...
# face: [ plane, v1,v2,v3,v4... ]
# plane = [nx, ny, nz, d], or simply [a, b, c, d], do not need to be normalized.
# v1 = [x, y, z], the cross point between the face and the 12 edges of the bounding box (at most 4 cross points)
...
def get_polygon_from_params(params):
    epsilon = 1e-5
    face = [] # save the vertices that compose this face.
    a,b,c,d = params

    #print(f'[get_poly] plane = {params}')
    ...
    #detect intersection on the 12 edges of a box [-1,1]^3
    if abs(a)>=abs(b) and abs(a)>=abs(c):
        ...
        face.append([a,b,c,-d]) # face[0] is not a point ... ??? 
        if a>0:
    ...
    #print(f'[get_ploy] face = {face[1:]}')

    # should clamp face into [-1, 1] ???
    for i in range(1, len(face)):
        face[i] = np.clip(face[i], -border_limit, border_limit)
...
# target_face: or temp_face, the faces from get_polygon_from_params()
# face_group: an empty list at initial, the returned results.

def join_polygons(target_face, face_group):
    epsilon = 1e-5
    faces = []
    a,b,c,w = target_face[0] # face[0] is used for saving plane parameters
    ...
    for i in range(len(face_group)):
        ...
        # loop points in face_i, detect whether it is inside target_face or not.
        ...
        # face_i is crossed with target_face, need to split face_i
        ...
        # face_i is on the front, so keep the original face_i
        elif front_flag:
            faces.append(face_i)
...
def digest_bsp(bsp_convex,bias):
    ...
    # each face is [plane, v1, v2, v3, ...], at least 3, at most 4 points !
...
# further merge same vertex by a threshold (so the mesh is watertight)
def get_mesh_watertight(bsp_convex_list):
    ...
    for k in range(len(bsp_convex_list)):
        ...
        #vg, tg = digest_bsp_robust(bsp_convex_list[k],bias=0)
