...
# to voxelize an implicit 3d representation (DIF), in a sparse query way.
# * dense query all voxels at `resolution_0`.
# * only subdivide active voxels (e.g., occupied voxels) into 8 sub-voxels.
# * sparse query the sub-voxels.
# * repeat until all active voxels at `resolution_0 * 2 ^ depth` are queried.
...
cdef class MISE:
    ...
    def __cinit__(self, int resolution_0, int depth, double threshold):
        self.resolution_0 = resolution_0 # input resolution, e.g., 16 --> [16, 16, 16]
        self.depth = depth # upsampling_steps e.g., 3
        self.threshold = threshold # to determine whether a voxel is active (so we will subdivide it further)
        self.voxel_size_0 = (1 << depth) # upsampling_scale, e.g., (1 << 3) = 8
        self.resolution = resolution_0 * self.voxel_size_0 # output resolution e.g., 16 * (1 << 3) = 128
        ...
        # Create initial grid points (values are located on grid corners, instead of voxel centers.)
